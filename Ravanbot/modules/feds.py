...
import RAVAN-ROBOT.modules.sql.feds_sql as sql
from RAVAN-ROBOT import (
    DEV_USERS,
    LOGGER,
    MESSAGE_DUMP,
    OWNER_ID,
...
from RAVAN-ROBOT.modules.disable import DisableAbleCommandHandler
from RAVAN-ROBOT.modules.helper_funcs.alternate import (
    send_action,
    send_message,
    typing_action,
)
from RAVAN-ROBOT.modules.helper_funcs.chat_status import is_user_admin
from RAVAN-ROBOT.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from RAVAN-ROBOT.modules.helper_funcs.string_handling import markdown_parser
...
# Federation by TeamRavan 2018-2019
# Federation rework by Devil_Writer 2019
# Federation update v2 by TeamRavan 2019
#
# Time spended on feds = 10h by #TeamRavan
# Time spended on reworking on the whole feds = 22+ hours by @Devil_Writer
# Time spended on updating version to v2 = 26+ hours by @Devil_Writer
#
# Total spended for making this features is 68+ hours

# LOGGER.info("Original federation module by TeamRavan, reworked by Mr Kaushik (@Devil_Writer) on Telegram.")
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Elizabeth_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Elizabeth_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Elizabeth_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Elizabeth_fbanned_users.csv",
